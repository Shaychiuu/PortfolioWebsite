...
const Firefly_two = ({ position, scale }) => {
  ...
  useFrame(({ clock }) => {
    ...
    meshRef.current.position.y = position[1] + Math.sin(t) * 4;
    meshRef.current.position.z = position[2] + Math.cos(t) * -4;
  ...
  return (
    <a.group ref={meshRef} scale={scale}>
      ...
      <pointLight ref={lightRef} distance={5} intensity={2} color="yellow" />
    </a.group>
  );
};

export default Firefly_two;
